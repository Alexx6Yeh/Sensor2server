...
//        pairedListView.setOnItemClickListener(mDeviceClickListener);
    ...
    public void button_Yes(View v){
        ...
        for(int i=0;i< list.getCheckedItemPositions().size();i++) {
            address = datalist.get(list.getCheckedItemPositions().keyAt(i));
            pout += address.substring(address.length() - 17)+",";
        }
        CharSequence text2 = pout;
        int duration2 = Toast.LENGTH_SHORT;
        Toast.makeText(this, text2, duration2).show();

        // Create the result Intent and include the MAC address
        Intent intent = new Intent();
        intent.putExtra(EXTRA_DEVICE_ADDRESS, pout);
